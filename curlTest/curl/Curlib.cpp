...
//
Curlib oMyCurl;
ICurl *ICurl::GetInstance()
{
	return (ICurl *)&oMyCurl;
}
				...
				if(pstInfo->dwBlockSize>0 && pstInfo->dwHadWrite>=pstInfo->dwBlockSize)
				{
					...
					if(nPercent > pstInfo->nPercent)
					{
						...
						if(pstInfo->pListener)
							pstInfo->pListener->NotifyDownload(pstInfo->strID, pstInfo->nPercent);
					}
	...
	if(pstInfo && buffer)
	{
		...
		if(pstInfo->dwBlockSize>0 && pstInfo->dwHadWrite>=pstInfo->dwBlockSize)
		{
			...
			if(nPercent > pstInfo->nPercent)
			{
				pstInfo->nPercent = nPercent;
				if(pstInfo->pListener)
					pstInfo->pListener->NotifyUpload(pstInfo->strID, pstInfo->nPercent);
...
void Curlib::SetListener(ICurlNotify *pListener)
{
	m_pListener = pListener;
}

bool Curlib::Post(std::string strUrl, std::string strPost, std::string & strResponse)
{
	bool bRet = false;
...
bool Curlib::Get(std::string strUrl, std::string& strResponse)
{
	bool bRet = false;
...
bool Curlib::Download(std::string strUrl, std::string strDstPath, std::string strID)
{
	bool bRet = false;
	...
	if(m_pListener)
		m_pListener->NotifyDownload(strID,bRet?100:-1);
...
bool Curlib::Upload(std::string strUrl, std::string strSrcPath, std::string strID)
{
	bool bRet = false;
	...
	if(m_pListener)
		m_pListener->NotifyUpload(strID,bRet?100:-1);

	return bRet;
}

DWORD Curlib::GetRemoteFileSize(std::string strUrl)
{
...
DWORD Curlib::GetLocalFileSize(std::string strPath)
{
	DWORD dwFileSize = 0;

	WIN32_FIND_DATAA fileInfo; 
	HANDLE hFind = FindFirstFileA(strPath.c_str(), &fileInfo); 
...
bool Curlib::GetFileName(std::string strPath, std::string& strName)
{
	bool bRet = false;
